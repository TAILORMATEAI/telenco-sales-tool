...
export default function Header({ hideProfileMenuContext = false, actionButton }: HeaderProps) {
  ...
  const themeBgClass = isTelenet ? 'bg-[#FFC421]' : 'bg-[#E74B4D]';
    ...
    <header className="px-6 py-5 flex justify-between items-center max-w-7xl mx-auto w-full relative z-40 mt-3">
      <div className="flex items-center">
        <img
          ...
          alt="Telenco Logo"
          className="h-6 sm:h-8 object-contain opacity-90 transition-opacity hover:opacity-100 cursor-pointer"
          ...
          {getDisplayName() && (
            <div className="relative">
              ...
              >
                ...
                <div className={`w-11 h-11 rounded-full overflow-hidden shrink-0 ${themeBgClass} flex items-center justify-center`}>
                  {profile?.avatar_id ? (
                    <img src={profile.avatar_id} alt={getDisplayName()} className="w-full h-full object-cover" />
                  ) : (
                    <span className="text-white text-sm font-bold">{getDisplayName().charAt(0).toUpperCase()}</span>
                  )}
                </div>
                <span className="hidden sm:inline text-sm font-bold text-white tracking-tight pl-2 pr-3">
                  {getDisplayName().split(' ')[0]}
                </span>
                <ChevronDownIcon className={`w-4 h-4 text-white/80 transition-transform duration-300 ml-2 mr-4 sm:ml-0 sm:mr-3 ${isProfileMenuOpen ? 'rotate-180' : ''}`} />
              </button>
                  ...
                  >
                    <div className="px-4 py-3 border-b border-slate-50 mb-1 flex items-center justify-between">
                      <div className="flex-1 min-w-0 pr-2">
                        ...
                        <p className="text-sm font-bold text-slate-600 mt-1.5 truncate">{getDisplayName()}</p>
                      </div>
                      <div className="flex bg-slate-100 p-0.5 rounded-full border border-slate-200 shrink-0">
                        {['NL', 'FR'].map((l) => (
                          <button
                            ...
                            className={`px-3 py-1 text-[10px] font-bold rounded-full transition-colors ${lang === l ? `${themeBgClass} text-white shadow-sm` : 'text-slate-500 hover:text-slate-600'}`}
                          >
                            {l}
                          </button>
                        ))}
                    ...
                    <button
                      ...
                      className="flex items-center gap-2.5 px-4 py-2.5 text-sm font-medium text-slate-600 hover:bg-slate-50 hover:text-slate-500 transition-colors text-left w-full"
                    >
